...
```sh
((sudo apt-get update && sudo apt-get install -y curl) || (sudo dnf install -y curl) || (sudo pacman -Sy --noconfirm curl)) && curl -sSL https://raw.githubusercontent.com/Operators-Diaries/posaune/main/scripts/start.sh | bash
```
